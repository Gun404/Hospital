...
public class JwtTokenPatientInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            Claims claims = JwtUtil.parseJWT(jwtProperties.getPatientSecretKey(), token);
            String patientName = claims.get(JwtClaimsConstant.PATIENT_NAME).toString();
            log.info("患者姓名：{}",patientName );
            NameContext.setCurrentName(patientName);
            //3、通过，放行
            log.info("令牌合法，放行");
            return true;
        } catch (Exception ex) {
